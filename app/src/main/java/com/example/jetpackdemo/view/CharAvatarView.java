...
public class CharAvatarView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (null != text) {
            ...
            canvas.drawCircle((float) getMeasuredWidth() / 2, (float) getMeasuredWidth() / 2, (float) getMeasuredWidth() / 2, mPaintBackground);
            // 写字
            mPaintText.setColor(Color.WHITE);
            mPaintText.setTextSize((float) getMeasuredWidth() / 2);
            ...
            canvas.drawText(text, (float) getMeasuredWidth() / 2, baseline, mPaintText);
        }
    }
